...
class Subscription(models.Model):
    ...
    subscribed_to = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='subscribers', verbose_name='Подписан на')
    ...
    def __str__(self):
        return f"Подписка {self.user.username} на {self.subscribed_to.username}"
...
class Recipe(models.Model):
    ...
    cooking_time = models.IntegerField(verbose_name='Время готовки (в минутах)')
    text = models.TextField(verbose_name='Описание')
    date_posted = models.DateTimeField(auto_now_add=True, verbose_name='Время публикации')
...
class ShoppingCartItem(models.Model):
    ...
    recipe = models.ForeignKey(
        Recipe, on_delete=models.CASCADE, related_name='in_shopping_carts', verbose_name='Рецепт')
