...
class SubscriptionTestCase(APIResponseTestCase):
    ...
    def test_subscribe(self):
        self.assert_response(
            f'/api/users/{self.user1.pk}/subscribe/', method='post',
    ...
    def test_subscribe_not_found(self):
        self.assert_response(
            '/api/users/99999999/subscribe/', method='post',
    ...
    def test_already_subscribed(self):
        self.assert_response(
            f'/api/users/{self.user3.pk}/subscribe/', method='post',
            expected_status=status.HTTP_400_BAD_REQUEST,
            login_as=self.user1)

    def test_unsubscribe(self):
        self.assert_response(
            f'/api/users/{self.user3.pk}/subscribe/', method='delete',
            expected_status=status.HTTP_204_NO_CONTENT,
            login_as=self.user1)

    def test_unsubscribe_not_found(self):
        self.assert_response(
            '/api/users/99999999/subscribe/', method='delete',
            expected_status=status.HTTP_404_NOT_FOUND,
            login_as=self.user1)

    def test_unsubscribe_no_auth(self):
        self.assert_response(
            f'/api/users/{self.user1.pk}/subscribe/', method='delete',
            expected_status=status.HTTP_401_UNAUTHORIZED)
